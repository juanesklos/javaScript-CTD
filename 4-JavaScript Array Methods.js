...
//---------- QUESTION 1 ----------
// Create an variable called 'names' and assign it an array of people's names.  
// Write a function called 'printNames' that uses the forEach array method to log each name in an array to the console.  
// Remember, since you're using the console.log in the function, you'll just want to call your function and pass it the array of names when testing your code.
...
console.log("Why apper undefined at the end of the list?");
...
console.log();
//---------- QUESTION 3 ----------
// Create a variable called 'myNumbers' and assign it an array of numbers.  
// Now write a function called 'totalPoints' that uses forEach to add up all the numbers in that array of numbers.
...
let myNumbers = [10, 2, 7, 5, 8];

let totalPoints =  function(arrayAdd){
    let count = 0;
    arrayAdd.forEach(number => {
        count += number;
    })
    return count;
...
//---------- QUESTION 4 ----------
// Create a variable called 'myWords' and assign it an array of words.  
// Then write a function called 'buildSentence' that takes in an array of words and uses forEach to add the strings together. 
// It should also add a space, " ", after each word.
...
let buildSentence = function(arrayWords){
    let stringout = "";
...
//---------- QUESTION 5 ----------
// Create a variable called 'decimals' and assign it an array of decimal numbers.  
// Write a function called 'logPercentages' that takes an array of decimal numbers 
...
let logPercentages = function(arrayDecimals){
    arrayDecimals.forEach(number => {
        calculateNumber = (number * 100) + "%";
...
let arrayResponse = [];
let count = 0;
let addThreeToAll = function(){
    
    startingNums.map( number => {
        arrayResponse[count] = number + 3;
        count++;
    });
...
console.log();
// ---------- QUESTION 7 ----------
// Create a variable called 'baseNums' and assign it an array of numbers.  
// Write a function called 'squareAll' that takes in an array of numbers, and uses map to return an array containing the squares of each of the numbers.  
// Remember, you can square a number by multiplying it by itself, or by using the exponent operator like so:
// let number = 4
// number \* number // 16
// number \*\* 2 // 16
...
let baseNums = [4, 41, 832, 72, 89];
let count2 = 0;
let squareAll = function(arraySquare){
    
    arraySquare.map( number => {
        arrayResponse[count2] = number * number;
        count2++;
    });
...
// ---------- QUESTION 8 ----------
// Write a function called 'allGreetings' that takes an array of names (strings) by using your 'names' variable from question 1. 
// Return an array of greetings (strings). The greetings should each be "Hello, [name], nice to meet you!".  You might find it helpful to use template strings.
...
let arrayResponse2 = [];
let count3 = 0;
let allGreetings = function(arrayParameter){

    arrayParameter.map(name => {
        names[count3] = `Hello, ${name}, nice to meet you!`;
        count3++;
    });
...
console.log();
// ---------- QUESTION 9 ----------
// Create a variable called 'users' and assign it an array of objects.  
// See example on line 153 below.  Write a function called 'getUsernames' that takes in an array of user objects and uses map to return an array of just the usernames.
...
let getUsernames = function(){
    ...
    users.map(user => {
        arraysUserNames.push(`${user.username}`)
    });

    return arraysUserNames;
...
// and uses the map method to return an array of the values at that key for each of the objects.  For example, 
// if we used the users array from the last exercise, we could do pluck(users, 'username') to get the same results as the last question.  
...
let arrayResponseObjetos = [];
let pluck = function(arrayParam, keyParam){
    arrayParam.map(user => {
        arrayResponseObjetos.push(user[keyParam]);
    });
    return arrayResponseObjetos;
...
console.log("Q10 Filter Method-------Start---------");

let arrayFilterResponseObjetos = [];
let pluckFilter = function(arrayParam, keyParam){
   arrayFilterResponseObjetos = arrayParam.filter(user => user.isAdmin === keyParam);
    return arrayFilterResponseObjetos;

};

console.log (`Q10 pluckFilter "isAdmin":`, pluck(users, "isAdmin"));
console.log("Q10-------End---------");
console.log();
...
// ---------- QUESTION 11 ----------
// Write a function called 'evenNumbers' that takes an array as an argument and uses the filter method.  
// Return only the even numbers from the array of numbers.  Remember - you can find out if a number is even by using the % operator (modulus operator) like this:
...
let evenNumbers = function(arrayParameter){
    let arrayEvenResponse = arrayParameter.filter(number => number % 2 === 0);
    return arrayEvenResponse;
};
...
let greaterThan100 = function(arrayNumbers){
    let responseGreaterThan100 = arrayNumbers.filter(number => number > 100);
    return responseGreaterThan100;
...
// ---------- QUESTION 13 ----------
// Write a function called 'nonAdminUsers' that takes in an array of user objects.  
// The objects should have a username and isAdmin property each like they do in Question 9.  
...
let nonAdminUsers = function(arrayObjects){
    let responseNonAdmin = arrayObjects.filter(rol => rol.isAdmin === false);
    return responseNonAdmin;
};
...
console.log();
// ---------- QUESTION 14 ----------
// Write a function called 'countAdminUsers' that uses filter and the length method on the result from filter.  
// The function should return the number of users that have 'isAdmin: true' as part of their object.
// EXAMPLE CALL: (using users array from Q9)
...
let countAdminUsers = function(arrayParameter){
    let reponseIsAdmin = arrayParameter.filter(rol => rol.isAdmin === true );
    return reponseIsAdmin.length;
};
...
// ---------- QUESTION 15 ----------
// Write a function called 'shorterThanX' that takes two arguments: an array of strings and an integer.  
// Use the filter method to return all the strings in the array whose length is shorter than the integer provided as an argument.  
...
let shorterThanX = function(arrayStringsParam, integerParam){
    let responseStrings = arrayStringsParam.filter(text => text.length < integerParam);
...
// ---------- QUESTION 16 ----------
// Write a function called 'onlyStrings' that takes an array that has different types of elements (numbers, booleans, strings, etc.) 
// and uses the filter method to return an array of only the strings.
...
let onlyStrings = function(arrayOnlyStrings){
    let resultStrings = arrayOnlyStrings.filter(element => typeof element === "string");
...
// ---------- QUESTION 17 ----------
// Write a function called 'firstOdd' that takes an array of numbers as an argument.  
// The function should use the find method to find the first odd number in an array.  
...
let firstOdd = function(arrayNumbersParam){
    const firstOdd = arrayNumbersParam.find(number => number % 2 === 1);
    return firstOdd;
...
// ---------- QUESTION 18 ----------
// Write a function called 'divisibleByTen' that takes an array of numbers as an argument.  
// The function should use the find method to return the first number that is a multiple of 10.  
// If there are no numbers divisible by 10, return undefined.
...
// PUT YOUR CODE HERE
console.log("Q18-------Start---------");

let divArray = [4,41,832,72,89,120,431,505,70];

let divisibleByTen = function(arrayParameter){
    return arrayParameter.find(number => number % 10 === 0);
...
// ---------- QUESTION 19 ----------
// Write a function called 'divisibleByX' that takes two arguments: an array and an integer.  
// The function should use the find method to return the first number in the array that's divisible by the integer that was passed as an argument.  
// If there are no values that are divisible by the second argument, return undefined.
...
// PUT YOUR CODE HERE
console.log("Q19-------Start---------");


let numbers = [4,41,832,72,89,120,431,70];

let divisibleByX = function(arrayParameter, integerParam){
    return arrayParameter.find(number => number % integerParam === 0);
...
// The function should use the find method to return the first string in the array that starts with the letter provided as the argument.  
// If there's no string in the array that starts with that letter, return undefined. The function should leave lowercase and uppercase letters alone.
...
let startsWithLetter = function(arrayParameter, letterParam){
    if(letterParam.length > 1) return "Letter must be a string of length 1";
    return arrayParameter.find(word => word.charAt(0) === letterParam);
};
